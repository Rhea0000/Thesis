...
def result_processing(result, keyword):
    try:
        ...
        # If it's not JSON, try to extract text after the specified keyword
        try:
...
if hasattr(llm, 'model_name') and 'huggingface' in llm.model_name:
    ResultQ1 = result_processing(ResultQ1,keyword="Assistant")
...
if isinstance(ResultQ1, dict):
    ...
    for term, definition in ResultQ1.items():
        ...
        if hasattr(llm, 'model_name') and 'huggingface' in llm.model_name:
            ResultQ234 = result_processing(ResultQ234,keyword="Assistant")
...
if ResultQ1 is not None:
    ...
    if hasattr(llm, 'model_name') and 'huggingface' in llm.model_name:
        ResultQ5 = result_processing(ResultQ5,keyword="Assistant")
    elif hasattr(llm, 'model_name') and 'gpt' in llm.model_name:
        pass
    else:
        pass
...
if ResultQ5 is not None:
    ...
    if hasattr(llm, 'model_name') and 'huggingface' in llm.model_name:
        ResultQ6 = result_processing(ResultQ6,keyword="Assistant")
    elif hasattr(llm, 'model_name') and 'gpt' in llm.model_name:
        pass
    else:
        pass
